...
module.exports = {
  ...
  apply(ctx) {
        ...
        rpc('list-songs').then((res) => {
          ...
          if (firstLoad) {
            ...
            if (this.audioEl) {
              ...
              this.audioEl.play().then(() => { this.playing = true; this.notify() }).catch(() => {})
            }
          }
          this.notify()
        }).catch(() => {})
    ...
    function MiniPlayer() {
      ...
      return React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '4px', padding: '2px 4px', maxWidth: '100%' } },
        React.createElement('audio', {
          ref: (el) => player.setAudio(el),
          onEnded: () => player.onEnded(),
          onPlay: () => { player.playing = true; player.notify() },
          onPause: () => { player.playing = false; player.notify() },
      ...
      return React.createElement('div', null,
        ...
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 0' } },
          ...
          React.createElement('span', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } },
            player.songs.length > 0 ? player.songs[player.current].title : '加载曲目…'),
        ),
